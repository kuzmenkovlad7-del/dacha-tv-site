...
export function Navigation({ phone, phoneSecondary, siteSettings, logoPath }: NavigationProps) {
  ...
  // Mount flag — needed so createPortal only runs client-side
  ...
  // The menu panel — portalled to document.body so it escapes the header's
  // backdrop-filter stacking context (which would otherwise contain fixed children)
  const menuPanel = (
    <div
    ...
    >
      ...
      <div className="flex-shrink-0 px-5 pt-4 pb-6 border-t border-gray-100 space-y-4 bg-white"
        ...
        {/* Phone numbers — same size for both */}
        <div className="flex flex-col gap-2">
          <a href={`tel:${formatPhoneTel(resolvedPhone)}`}
            className="flex items-center gap-2.5 h-11 text-bark font-semibold text-[0.9375rem]">
  ...
  return (
    <>
      ...
      {/* Portalled to body — escapes the header's backdrop-filter stacking context */}
