...
function slotsFromMedia(media: ProductMedia[]): {
  ...
  const images = media
    ...
    .map((m) => ({ id: m.id, url: m.url, preview: m.url, alt: m.alt ?? '', isPrimary: m.is_primary }))
  ...
  const videos = media
    ...
    .map((m) => ({ id: m.id, url: m.url, preview: m.url }))
...
export function MediaManager({ initialMedia, productName = '' }: MediaManagerProps) {
  ...
  const handleImgFile = useCallback((i: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    const preview = URL.createObjectURL(file)
    setImages((s) => s.map((slot, j) => (j === i ? { ...slot, preview } : slot)))
  }, [])

  const handleVidFile = useCallback((i: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file) return
    const preview = URL.createObjectURL(file)
    setVideos((s) => s.map((slot, j) => (j === i ? { ...slot, preview } : slot)))
  }, [])

  const addImage = () =>
    setImages((s) => [...s, { id: null, url: '', preview: '', alt: '', isPrimary: s.length === 0 }])
  ...
  const addVideo = () => setVideos((s) => [...s, { id: null, url: '', preview: '' }])
        ...
        <div className="space-y-4">
          {images.map((slot, i) => (
            <div key={i} className="border border-gray-100 rounded-xl p-3 space-y-2 bg-gray-50/50">
              ...
              <input type="hidden" name={`img_id_${i}`} value={slot.id ?? ''} />
              ...
              <details className="text-xs">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setImages((s) =>
                      s.map((sl, j) =>
                        j === i ? { ...sl, url: val, preview: val.startsWith('blob:') ? sl.preview : val } : sl,
                      ),
                    )
                  }}
        ...
        <div className="space-y-3">
          {videos.map((slot, i) => (
            <div key={i} className="border border-gray-100 rounded-xl p-3 space-y-2 bg-gray-50/50">
              ...
              <input type="hidden" name={`vid_id_${i}`} value={slot.id ?? ''} />
              ...
              {slot.url && !slot.preview.startsWith('blob:') && (
                <p className="text-xs text-gray-400 truncate" title={slot.url}>{slot.url}</p>
              )}
              <input
                type="file"
                name={`vid_file_${i}`}
