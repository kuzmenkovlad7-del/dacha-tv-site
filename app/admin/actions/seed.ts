...
export async function syncCatalog(): Promise<SyncResult> {
  ...
  try {
    ...
    // Honey — upsert by slug (6 products); ignoreDuplicates:false = always update canonical data
    ...
    const apiaryRows = STATIC_APIARY.map(({ id: _id, ...rest }) => rest)
    ...
    const beekeeperRows = STATIC_BEEKEEPER.map(({ id: _id, ...rest }) => rest)
