...
import {
  ...
  bulkActivateFromSupplierAction,
} from './actions'
import type { CatalogCategory } from '@/types'

export const metadata: Metadata = {
  title: 'Адмін — Категорії каталогу',
...
export default async function AdminCatalogCategoriesPage() {
  ...
  try {
    const [cats, scCount] = await Promise.all([
      client.from('catalog_categories').select('*').order('display_order').order('name_ua'),
    ...
    <div className="px-4 sm:px-6 py-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          {publishedCount} публічних · {categories.length - publishedCount} чернеток ·
          {supplierCategoryCount} категорій постачальника доступно
        </p>
      </div>
      ...
      {tablesMissing && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-6">
          ...
          <p className="text-sm text-amber-700">Запустіть міграцію 039 у Supabase SQL Editor.</p>
        </div>
      )}
      ...
      {!tablesMissing && supplierCategoryCount > 0 && categories.length === 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          ...
          <p className="text-sm text-blue-700 mb-3">
            Активуйте перші N категорій автоматично, потім відредагуйте SEO-поля вручну.
          </p>
                ...
                return (
                  <tr key={cat.id} className="hover:bg-gray-50/50">
                    <td className="px-5 py-3 font-medium text-gray-900">
                      {cat.name_ua}
                      {cat.description && (
                        <p className="text-xs text-gray-400 font-normal truncate max-w-xs">{cat.description}</p>
                      )}
                    </td>
                    ...
                    <td className="px-5 py-3 text-center">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        cat.is_published ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                      }`}>
                    ...
                    <td className="px-5 py-3 text-right flex items-center justify-end gap-3">
                      {cat.is_published ? (
                        <form action={unpub}>
                          <button type="submit" className="text-xs text-gray-500 hover:text-gray-800 font-medium">Зняти</button>
                        </form>
                      ) : (
                        <form action={pub}>
                          <button type="submit" className="text-xs text-green-700 hover:text-green-900 font-medium">Опублікувати</button>
                        </form>
                      )}
                      <form action={del} onSubmit={(e) => { if (!confirm('Видалити категорію?')) e.preventDefault() }}>
                        <button type="submit" className="text-xs text-red-500 hover:text-red-700 font-medium">Видалити</button>
                      </form>
                    </td>
                  </tr>
                )
      ...
      {!tablesMissing && (
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-6">
          <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Додати категорію</h2>
