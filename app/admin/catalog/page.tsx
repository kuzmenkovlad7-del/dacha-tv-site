...
export default async function AdminCatalogPage() {
  ...
  try {
    ...
    if (catalog.error?.message.includes('does not exist') || catalog.error?.message.includes('relation')) {
      ...
      publishedCount = catalogProducts.filter((p) => p.status === 'published').length
      ...
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {publishedCount} опублікованих · {catalogProducts.length - publishedCount} у чернетці
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 bg-gray-50">
            <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
              Каталог ({catalogProducts.length})
            </h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
